...
import {
  ...
  Badge, ProgressBar, Banner, Button, ButtonGroup, Box, List, Modal, Icon,
...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  // Step 1: Authenticate (always required)
  const { session } = await authenticate.admin(request);
  const shopDomain = session.shop;

  console.log("[AltOptimizer] Dashboard loader started", { shopDomain });

  // Step 2: Get or create shop record
  let shop;
  try {
    shop = await getOrCreateShop(shopDomain);
    console.log("[AltOptimizer] Shop resolved", { shopId: shop.id, planType: shop.planType });
  } catch (error: any) {
    console.error("[AltOptimizer] getOrCreateShop failed:", error.message, error.stack);
    shop = { id: 0, planType: "free", shopDomain };
  }

  // Step 3: Get dashboard stats (with safe defaults)
  let stats = {
    totalProducts: 0,
    totalImages: 0,
    imagesWithAlt: 0,
  ...
  try {
    if (shop.id > 0) {
      stats = await getDashboardStats(shop.id);
      console.log("[AltOptimizer] Dashboard stats loaded", stats);
    }
  } catch (error: any) {
    console.error("[AltOptimizer] getDashboardStats failed:", error.message, error.stack);
  }

  // Step 4: Get usage info (with safe defaults)
  let usage = {
    imagesGenerated: 0,
    tagsGenerated: 0,
    jsonLdGenerated: 0,
  ...
  try {
    if (shop.id > 0) {
      usage = await getCurrentUsage(shop.id);
      console.log("[AltOptimizer] Usage loaded", usage);
    }
  } catch (error: any) {
    console.error("[AltOptimizer] getCurrentUsage failed:", error.message, error.stack);
  }
